...
const verifierExpirationEtEnvoyerNotifications = async () => {
  try {
    ...
    const decisionsToNotify = await UssdDecision.findAll({
      where: {
        date_expiration: {
    ...
    if (decisionsToNotify.length > 0) {
      ...
      for (const attributionId in decisionsGroupedByAttribution) {
        ...
        const clientName =
          decision.USSDAttribution && decision.USSDAttribution.Client
            ? decision.USSDAttribution.Client.denomination
        ...
        if (activeRenewalDecisions.length > 0) {
          ...
          const UssdDecision = decisions.find(
            (decision) => decision.type_decision === "attribution"
          );
          decisionToNotify = UssdDecision;
        }
    ...
    const expiredDecisions = await UssdDecision.findAll({
      where: {
        date_expiration: {

...
const login = async (req, res) => {
  ...
  try {
    ...
    if (ldapResponse && ldapResponse.success) {
      // LDAP réussi, vérifier utilisateur en base
      const utilisateur = await Utilisateur.findOne({ where: { username } });
      if (!utilisateur) {
        return res.json({
          success: false,
      ...
      return res.json({
        success: true,
        message: "Connexion réussie via LDAP.",
        user: utilisateurSansPassword,
        token
      });
    } else {
      // LDAP échoué, essayer bcrypt
      const utilisateur = await Utilisateur.findOne({ where: { username } });
      if (!utilisateur) {
        return res.json({
          success: false,
      ...
      return res.json({
        success: true,
        message: "Connexion réussie via mot de passe.",
        user: utilisateurSansPassword,
        token
      });
    }
